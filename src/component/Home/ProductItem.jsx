...
function ProductItem() {
  return (
    <span className="product-item flex flex-col bg-[#fffffff5] w-[10.5rem] max-w-60 min-h-[375px] max-h-fit p-1.5 rounded-md flex-grow pb-2">
      {/**product image */}
      <span>
        <img className="w-full h-[10rem]" src={productImage}></img>
      </span>
      {/**Category  and product name*/}
      <span className="mt-2 flex flex-col pl-3 border-b-2 pb-2.5">
        <h5 className="text-lg m-0 text-gray-600">HOUSE</h5>
        <h5 className="text-xl font-bold m-0 text-gray-600">3 Bed Room Flat</h5>
      </span>
      {/**product pricing */}
      <span className="flex flex-col pl-3 mt-1">
        <h5 className="text-lg price-slash ">&#8358;13,000,000</h5>
        <h5 className="text-lg text-green-700">&#8358;9,000,000</h5>
      </span>
      {/**stock status */}
      ...
      <span className="w-full block mt-1 pl-2 pr-2 overflow-hidden">
        <span className="bg-[#e2a83dab] w-full h-10 rounded-3xl flex pl-3.5 overflow-hidden p-1">
          <h5 className="text-gray-800 font-bold text-[0.9rem]  mt-1.5">
            ADD TO CART
          </h5>
